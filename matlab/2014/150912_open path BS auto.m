...
dataDir = 'E:\EAPL\JW_Observation\보성\csv'; % folder name
% dataDir = 'E:\EAPL\JW_Observation\은평뉴타운\_csv'; % folder name
dataName = dir(fullfile(dataDir, 'CSV_8972.*'))'; 
...
%%
for t=2:length(dataName) %n_st:n_en % 여러개의 CSV 파일을 연속적으로 처리하기 위한 loop
...
%             if ((mod(data(i,3),100)==0)||(mod(data(i,3),100)==30))&&(mod(data(i,4),1)==0.1)
    ...
    %% result
    result = zeros(max(num_30min_n(:,1)),36);
    ...
    % 33; w_bar (m/s)
    % 34; Tk (degree C)
    % 35; signal strength of CO2
    % 36; signal strength of H2O
    ...
    for i = 1:max(num_30min_n(:,1))
        ...
        for j = 1:num_30min_n(i,2)
            if i ==1
                ...
                temp(j,9) = data(j,po_sgst_CO2);
                temp(j,10) = data(j,po_sgst_H2O);
            else
                ...
                temp(j,9) = data(num_30min_n(i-1,3)+j,po_sgst_CO2);
                temp(j,10) = data(num_30min_n(i-1,3)+j,po_sgst_H2O);
            end
        end
        clear j

        result(i,35) = mean(temp(:,9));
        result(i,36) = mean(temp(:,10));
    ...
    clear i temp
    clear size_n size_var
end
close(wait);
clear data dataDir dataName num_30min_n result wait t
clear po_u po_v po_w po_cell_tmpr po_cell_prs po_Ts po_H2O po_CO2 po_sgst_CO2 po_sgst_H2O sonic_ang 
