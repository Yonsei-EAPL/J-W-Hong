...
DOM = [31;28;31;30;31;30;31;31;30;31;30;31]; % day of month for 2011
% Import the Result
gpp = nc_varget('control11.monthly.nc','gpp_gb');
npp = nc_varget('control11.monthly.nc','npp_gb');
% respp = nc_varget('control11.monthly.nc','resp_p_gb');
% resps = nc_varget('control11.monthly.nc','resp_s_gb');
% isoprene = nc_varget('control11.monthly.nc','isoprene_gb');
% frac = nc_varget('control11.monthly_input.nc','frac');
...
%         isoprene_g(i,add_result(j,1),add_result(j,2))=isoprene_g(i,add_result(j,1),add_result(j,2))+isoprene(i,j);
...
YR_npp = zeros(360,720);
YR_npgp = zeros(360,720);
for i = 1:360
    for j = 1:720
        YR_npp(i,j) = con_npp(13,i,j);
        YR_npgp(i,j) = YR_npp(i,j)/YR_gpp(i,j);
    end
end
clear i j k
...
% figure(5)
% contourf(YR_npgp,100,'LineStyle','none')
% hold on
% plot(m_lon(:,1),m_lat(:,1),'k','LineWidth',1.2)

npgp_lat = zeros(360,2);
temp = 0;
for i = 1:360
    lat = (i-1)*0.5 -89.75;
    npgp_lat(i,1) = lat;
    for j = 1:720
        if YR_gpp(i,j) >0
            temp = temp + 1;
            npgp_lat(i,2) = npgp_lat(i,2) + YR_npgp(i,j);
        end
    end
    npgp_lat(i,2) = npgp_lat(i,2)/temp;
    temp = 0;
end
clear i j 
