...
% 131023 Mr.Je-Woo Hong; Modify some equation, Remove WPL (Closed-path).


%% using data (130625 data from Seoul Forest, involving Card Error)
...
for i = 1:max(num_30min_n(:,1))
    ...
    q = mean(temp(:,6)); % mmol/mol
    Tk = (Ts + 273.15)/(1+0.000321*q); % K
    cell_prs = result(i,13); % hPa
    cell_tmpr = mean(temp(:,7)); % degree C 
    e = (q*10^(-3))*cell_prs; % hPa
    Pd = cell_prs - e; % hPa
    Rv = 461.495; % J/kg/K
    Rd = 287.05; % J/kg/K
    ...
    for j = 1:num_30min_n(i,2)
        ...
        cov_wq = cov_wq + (temp(j,3)-w_bar)*(temp(j,6)-q); % mmol/mol *m/s
    end
    clear j
    ...
    Mw = 18.015; % g/mol
    V = 0.082*Tk*10^(-3)/(cell_prs/1013.25); % m3/mol
    LE = lambda*Mw*cov_wq/(V)/1000; 
    result(i,18) = lambda;
    result(i,19) = LE;
    
    % co2 flux ; AmeriFlux Standard
    co2 = mean(temp(:,5)); % umol/mol
    cov_wc = 0;
    for j = 1:num_30min_n(i,2)
        cov_wc = cov_wc + (temp(j,3)-w_bar)*(temp(j,5)-co2);
    end
    clear j 
    cov_wc = cov_wc/num_30min_n(i,2); % umol*m/mol/s
    Fco2 = cov_wc/V; % umol/m2/s 
    result(i,20) = Fco2;
...
size_x = max(num_30min_n(:,1));
size_y = 34;
ncid = netcdf.create('result.nc','NC_WRITE');
length = netcdf.defDim(ncid,'length',size_x);
var = netcdf.defDim(ncid,'var',size_y);

idresult = netcdf.defVAR(ncid,'result','NC_FLOAT',[var length]);
netcdf.putAtt(ncid,idresult,'units',' ');
netcdf.endDef(ncid);
 
netcdf.putVar(ncid,idresult,result);
netcdf.close(ncid);
clear size_x size_y length var ncid idresult
result = nc_varget('result.nc','result');
