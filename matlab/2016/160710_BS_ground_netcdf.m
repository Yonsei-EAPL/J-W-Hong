%time = nc_varget('BSground.nc','time');
% need :  data and timei
...
%     clear i
    ...
    size_time = a;
    ncid = netcdf.create('BSground.nc','NC_WRITE');
    ...
    netcdf.putAtt(ncid,idCO2,'units','mmol mol-1(dry air)');
    idH2O = netcdf.defVar(ncid,'H2O','NC_DOUBLE',[time]);
    netcdf.putAtt(ncid,idH2O,'units','mmol mol-1(dry air)');
