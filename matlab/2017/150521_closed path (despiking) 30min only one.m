...
%dataDir = 'e:\EAPL\JW_Observation\덕유산\20160322 홍진규교수님(덕유산플럭스_6월)\20150716'; % folder name % EP
%dataName = dir(fullfile(dataDir, 'CSV_5498.ts_data_*'))'; % Mt.덕유
%total_result=cell(length(dataName),2);
% wait = waitbar(0,'...Flux Data Process...');
% n_st = 141;
% n_en = 150;
for t=1:1%length(dataName) %n_st:n_en% 여러개의 CSV 파일을 연속적으로 처리하기 위한 loop
...
%     clear i num_30min
    %% result
    result = zeros(1,62);%max(num_30min_n(:,1)),62);
    % 1; n_data (unitless)
    ...
    %% main process
    for i = 1:1%max(num_30min_n(:,1))
%         waitbar(((t-n_st)/(n_en-n_st)),wait,sprintf('%3f',i))
%         waitbar((t/length(dataName)),wait,sprintf('%3f',i))
        result(i,1) = 18000;%18000;    
        if result(i,1)==18000%18000==18000
            % extract
            temp = zeros(18000,10);%18000,10);
            for j = 1:18000%18000
                if i == 1
...
%             clear temp_diag_n1 temp_diag_n2
            ...
            for j = 1:18000
                ...
                if abs(mean_wd-temp_wd)>180
                    if (mean_wd-temp_wd)<0
                        ...
                        temp_std_wd = temp_std_wd + (360 - mean_wd+temp_wd)^2;% mean 270, temp 60 << 150deg % JY change '-' to '+'
                    end
                else
                    temp_std_wd = temp_std_wd + (mean_wd-temp_wd)^2;
                end
            end
            temp_std_wd = (temp_std_wd/18000)^(0.5);
            ...
            clear j 
            cov_wc = cov_wc/18000; % umol/mol * m/s
...
clear ans des_sigma des_window po_diag1 po_diag2 t 