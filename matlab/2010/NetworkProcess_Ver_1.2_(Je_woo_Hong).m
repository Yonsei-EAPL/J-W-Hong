...
% Version 1.2 (2011-05-04, Je-woo Hong)
% 추가 및 수정 된 사항
% - Part4. 변수명 오타 수정(Xt)
% - Part6. 이후에 앞의 계산결과가 지워지는 것 방지(주석처리)
% - Part8. Graph 그리기 쉽게 전체 결과를 정렬해주는 부분 추가
% - data_set에 gap(-99999로 표시)이 존재할 경우, gap 만을 제외하고 분석을 수행하도록 변경함
...
% 뒤에 불필요한 변수를 제거합니다.
clear i j k avg_sample size_data num_data z
% Part1. 끝.
...
for i=1:num_var 
    ...
    for j=1:num_var
        ...
        for z=1:num_ano_data
            if ano_data_set(z,j)==-99999
                ano_data_set(z,j)=99999;
            end
        end
...
% 불필요한 변수를 제거합니다.
clear pr_table_X pr_tag pr_table_Y pr_table_combine min_var_X min_var_Y max_var_X max_var_Y dif_var_X dif_var_Y dif_cl_X dif_cl_Y mi_sample i j k l m n z 
% Part2. 끝.
